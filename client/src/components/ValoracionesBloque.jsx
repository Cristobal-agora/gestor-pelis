...
const ValoracionesBloque = ({ tmdb_id, tipo, tmdb_score, trigger  }) => {
  ...
  return (
    <div className="valoraciones-bloque d-flex gap-4 align-items-center flex-wrap">
      <div><strong>⭐ TMDb:</strong> {tmdb_score?.toFixed(1) ?? 'N/A'}</div>
      <div><strong>👥 Usuarios:</strong> {mediaUsuarios ?? 'Sin datos'}</div>
      <div><strong>🧍 Tú:</strong> {miValoracion ?? 'Sin valorar'}</div>
    </div>
  );
};
